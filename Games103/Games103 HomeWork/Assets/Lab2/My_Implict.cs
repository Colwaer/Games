...
public class implicit_model1 : MonoBehaviour
{
	...
	void Get_Gradient(Vector3[] X, Vector3[] X_hat, float t, Vector3[] G)
	{
		...
		{
			G[i] = t * t * mass * (X[i] - X_hat[i]) - Physics.gravity * mass;
		}
		//Spring Force.
		for (int i = 0; i < E.Length; i += 2)
		{
			...
			G[point1] += spring_k *
				(1 - L[i / 2] / (X[point1] - X[point2]).magnitude) *
				(X[point1] - X[point2]);
			G[point2] -= spring_k *
				(1 - L[i / 2] / (X[point1] - X[point2]).magnitude) *
				(X[point1] - X[point2]);
